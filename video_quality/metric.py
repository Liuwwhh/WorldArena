...
def _compute_fid_overall(prepared_samples, device):
    _ensure_torch_hub_checkpoint(
        os.path.join(os.path.dirname(__file__), "models_downloaded", "pt_inception-2015-12-05-6726825d.pth")
    )
...
def _compute_fvd_overall(prepared_samples, frame_chunk_size, device):
    i3d_candidates = [
        os.path.join(os.path.dirname(__file__), "i3d_torchscript.pt"),
        os.path.join(os.path.dirname(__file__), "models_downloaded", "i3d_torchscript.pt"),
    ]
    i3d_path = next((p for p in i3d_candidates if os.path.isfile(p)), "")
    if not i3d_path:
        raise FileNotFoundError(f"FVD model not found. Tried: {i3d_candidates}")
...
def _compute_basic_metrics(prepared_samples, ctx: EvalContext):
    ...
    _ensure_torch_hub_checkpoint(
        os.path.join(os.path.dirname(__file__), "models_downloaded", "alexnet-owt-7be5be79.pth")
    )
